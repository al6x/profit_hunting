...
function plot_by_vol_by_rf(title, ds_vol, ds_rf; ylabel, solid_label=nothing, ydomain=nothing, y, y2=nothing, xscale=nothing, yscale=nothing)
  ...
  if solid_label is None:
    ftitle = f"{title} by (T, vol) and (T, rf)"
  else:
    ftitle = f"{title} by (T, vol) and (T, rf), (solid - {solid_label})"
