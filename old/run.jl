...
begin
  ...
  doc_before = """
    ...
    Run `julia mean/run.jl`.
  """

  doc_after = """
    # Data

    Data has both comission (large cap) and omission biases (no bankrupts), details `../hist_data/readme.md`.
...
function group_by_vol(ds, ops; volg=:vol_dc)
  ...
  DataFrame(rows)
  # r.nmmean  .= norm_mean.(r.mmean, r.period)
  # r.nmmean2 .= isnothing(model) ? nothing : norm_mean.(r.mmean2, r.period)
  # return r
end
...
function c_explore_mmeans(ds_trunc, ds_orig)
  ...
  plot_by_vol_by_rf(
    ...
    ylabel="Norm E[R]", solid_label="adjusted", y="nmean", y2="nmean2", ydomain=(0, 0.3)
  )

  plot_by_vol_rf(
    "Exploring Mean E[R]", means_vol_rf;
    ylabel="E[R]", solid_label="adjusted", y="mean", y2="mean2", ydomain=(1, nothing)
  )

  plot_by_vol_rf(
    "Exploring Norm E[R]", means_vol_rf;
    ylabel="Norm E[R]", solid_label="adjusted", y="nmean", y2="nmean2", ydomain=(0, 0.4)
  )
end
c_explore_mmeans(ds, ds_orig)


function c_explore_lmeans(ds_trunc, ds_orig)
  report("""
  # Exploring Log Mean E[log R]
  """)
  ...
  # plot_means(
  #   "Exploring Log Mean E[log R]"; ylabel="E[log R]", solid_label="adjusted",
  #   means_vol_rf=means_vol_rf, means_vol=means_vol, means_rf=means_rf,
  #   ydomain=(-0.05, 0.4), nydomain=(-0.05, 0.15)
  # )
  plot_by_vol_rf(
    "Exploring Log Mean E[log R]", means_vol_rf;
    ylabel="E[log R]", solid_label="adjusted",
    ydomain=(-0.05, nothing), nydomain=(-0.2, 0.3)
  )
end
c_explore_lmeans(ds, ds_orig)
