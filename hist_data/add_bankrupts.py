...
def add_bankrupts_unevenly(df, period_d, fields):
  ...
  bankrupts = {}
  # Needed to correctly acrount for cases when step < period_d, to avoid testing for bankruptcy multiple times during
  # the period.
  symbol_next_trial, period_d_delta = {}, pd.Timedelta(days=period_d)
  for _, row in df.iterrows():
    symbol, t = row['symbol'], row['t']
    if symbol in bankrupts or (symbol in symbol_next_trial and t < symbol_next_trial[symbol]):
      continue
    p_b = BANKRUPT_P[row['vol_dc']]
    if np.random.rand() < (1 - (1 - p_b) ** (period_d / 365)):
      b = row.copy()
      for field in fields:
